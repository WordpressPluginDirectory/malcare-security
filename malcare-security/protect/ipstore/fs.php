...
if (!class_exists('MCProtectIpstoreFS_V672')) :
	class MCProtectIpstoreFS_V672 {
		...
		function __construct() {
			...
			$ips = MCProtectUtils_V672::parseFile($ip_store_file);
		...
		public function getTypeIfBlacklistedIP($ip) {
			return $this->getIPType($ip, MCProtectIpstoreFS_V672::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPBlacklisted($ip) {
			return $this->checkIPPresent($ip, MCProtectIpstoreFS_V672::IP_TYPE_BLACKLISTED);
		}

		public function isFWIPWhitelisted($ip) {
			return $this->checkIPPresent($ip, MCProtectIpstoreFS_V672::IP_TYPE_WHITELISTED);
		}
		...
		private function getIPType($ip, $type) {
			switch ($type) {
			case MCProtectIpstoreFS_V672::IP_TYPE_BLACKLISTED:
				return isset($this->blacklisted_ips[$ip]) ? $this->blacklisted_ips[$ip] : null;
			case MCProtectIpstoreFS_V672::IP_TYPE_WHITELISTED:
