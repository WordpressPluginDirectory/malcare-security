...
class MCProtectIpstore_V639 {
	...
	function __construct($storage_type = MCProtectIpstore_V639::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V639::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V639();
		} else {
			$this->storage = new MCProtectIpstoreDB_V639();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V639::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V639::STORAGE_TYPE_DB) {
