...
class MCProtectIpstore_V672 {
	...
	function __construct($storage_type = MCProtectIpstore_V672::STORAGE_TYPE_DB) {
		$this->storage_type = $storage_type;
		if ($this->storage_type == MCProtectIpstore_V672::STORAGE_TYPE_FS) {
			$this->storage = new MCProtectIpstoreFS_V672();
		} else {
			$this->storage = new MCProtectIpstoreDB_V672();
		}
	}

	public static function uninstall() {
		MCProtectIpstoreDB_V672::uninstall();
	}

	public function isLPIPBlacklisted($ip) {
		if ($this->storage_type == MCProtectIpstore_V672::STORAGE_TYPE_DB) {
